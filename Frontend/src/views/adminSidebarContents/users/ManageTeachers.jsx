...
const ManageTeachers = () => {
  ...
  const stableFetchTeachers = useCallback(async (filters, token) => {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      if (result?.imported > 0) {
        ...
        Swal.fire(
          "Success",
          `Successfully imported ${count} teacher(s).`,
          "success"
        );

        // Reload the teacher list without refreshing the page
        setReloadTrigger(Date.now());
        console.log("Reload triggered", Date.now());
      }

      // If there are errors and teachers were also imported
      if (result?.errors && result.errors.length > 0 && result.imported > 0) {
        ...
        Swal.fire({
          title: "Import Errors",
          text: `${errorMessages}\nBut some teachers were successfully imported.`,
          icon: "warning",
        });
